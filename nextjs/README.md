...
> **v9.9.3** (2026-03-17) — 에이전트 UI 인터랙티브 개선 + 도구 탐색기 리뉴얼

| 영역 | 주요 변경 |
|------|-----------|
| **에이전트 정보** | 워커 뱃지 호버 팝업, 컨설팅 4단계 호버 설명, 사용법 안내 추가 |
| **도구 탐색기** | 호버 시 설명 펼침 + 매개변수/반환값/예시 질문 표시 |
| **레이아웃** | 채팅+사이드 flex 전환, 사이드 300px 고정, 빠른 분석 제거 |

> **v9.9.1** (2026-03-17) — 채팅 UX 개선 + FAQ 확장

| 영역 | 주요 변경 |
|------|-----------|
| **채팅 접기 버튼** | 헤더 상단 → 메시지 하단 중앙 이동 + "접기/펼치기" 텍스트 라벨 추가 |
| **사이드 FAQ 확장** | `카페24 FAQ` 4개 → 9개 + (RAG) 태그 추가 (배송비, 상품등록, 적립금, SEO, 도메인 등) |
