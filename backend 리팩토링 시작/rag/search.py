...
def _is_simple_query(query: str) -> bool:
    ...
    20자 이하 또는 한국어 명사+동사 조합의 짧은 질문은 LLM 호출 없이 스킵
    """
    q = query.strip()
    ...
    simple_patterns = [
        ...
        r'^[가-힣A-Za-z]+ ?왕국$',
        r'^[가-힣A-Za-z]+[은는이가] ?뭐',
        # 한국어 명사(+조사) + 동사/명사 조합 패턴 (예: "코호트 분석", "매출 추이", "셀러 현황")
        r'^[가-힣]{2,6}\s?[가-힣]{2,6}$',
        # "OO 알려줘", "OO 검색", "OO 조회" 등 짧은 명령형
        r'^[가-힣A-Za-z0-9 ]{2,15}\s?(알려줘|검색|조회|보여줘|확인)$',
        # "OO은/는/이/가 뭐야/뭔가요" 등
        r'^[가-힣A-Za-z]+[은는이가을를]\s?[가-힣]{2,6}$',
    ]
...
QUERY_EXPANSION_MAP: Dict[str, List[str]] = {
    # 이커머스 관련 — 일반적 동의어/관련어만 (특정 사명/제품명 제외)
    "정산": ["정산관리", "수수료", "환불"],
    "배송": ["배송설정", "배송방법", "택배", "출고"],
    "결제": ["결제수단", "결제방법", "결제설정"],
    "회원": ["고객", "사용자", "계정"],
