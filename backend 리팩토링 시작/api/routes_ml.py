...
import state as st
from api.common import verify_credentials, ModelSelectRequest, MarketingOptimizeRequest, error_response
...
def get_marketing_seller_info(seller_id: str, user: dict = Depends(verify_credentials)):
    try:
        ...
        if row.empty:
            return error_response(f"셀러 {seller_id}을(를) 찾을 수 없습니다")
        seller = row.iloc[0]
        shops = []
...
def optimize_marketing_budget(req: MarketingOptimizeRequest, user: dict = Depends(verify_credentials)):
    try:
        ...
        return error_response(f"마케팅 최적화 중 오류: {safe_str(e)}")
