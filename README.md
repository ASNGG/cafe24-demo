...
| **DB 보안 감시** | 룰엔진(<1ms) + Isolation Forest + SHAP 위험 요인 + LangChain Agent + Recovery Agent + Resend 이메일 알림 |
...
| **DB 보안 감시** | DB 대량 변경 실시간 차단 + SHAP 위험 요인 분석 + 복구 SQL 생성 | 룰엔진 + Isolation Forest + SHAP + LangChain Agent |
...
프로세스 발견(Counter 빈도 분석), 병목 분석(IQR), AI 자동화 추천(GPT-4o-mini), 다음 활동 예측(RandomForest), 이상 프로세스 탐지(IsolationForest)
...
| `analyst` | `analyst123` | 분석가 | 에이전트, 대시보드, 분석, 실험실, DB 보안 감시 |
| `user` | `user123` | 사용자 | 에이전트, 대시보드, 분석, 실험실, DB 보안 감시 |
